...
export default async (req: Request, context: Context) => {
  ...
  const id = randomUUID();
  await db.sql`
    INSERT INTO reviews (id, name, company, review, photo, rating, status, sort_order, updated_at, submitted_at)
    VALUES (${id}, ${name}, ${company}, ${review}, '', ${rating}, 'pending', 0, NOW(), NOW())
  `;
